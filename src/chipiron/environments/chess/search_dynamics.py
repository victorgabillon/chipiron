...
ChessStateT = TypeVar("ChessStateT", bound=ChessState)


@dataclass(frozen=True)
class ChessCopyStackSearchDynamics(SearchDynamics[ChessStateT, MoveKey]):
    """Wrap search dynamics with depth-aware board-copy policy in ``step`` only."""

    __anemone_search_dynamics__ = True

    base: SearchDynamics[ChessStateT, Any]
    ...
    def legal_actions(self, state: ChessStateT) -> Any:
        """Return legal action collection for ``state``."""
        return self.base.legal_actions(state)

    def action_name(self, state: ChessStateT, action: MoveKey) -> str:
        """Convert an action key to a stable branch name."""
        return self.base.action_name(state, action)

    def action_from_name(self, state: ChessStateT, name: str) -> MoveKey:
        """Convert a stable branch name to an action key."""
        return cast("MoveKey", self.base.action_from_name(state, name))
    ...
    def step(
        ...
        *,
        ...
        next_state = type(state)(board_copy)
        return Transition(
