...
class RecurVertiZipfTree(MoveAndValueTree):
    def create_tree_node(self, board, half_move, count, father_node):
        return VisitsAndProportionsNode(board, half_move, count, father_node, self.zipf_style)


class RecurVertiZipf(TreeAndValue):

    def __init__(self, arg):
        super().__init__(arg)

    def create_tree(self, board):
        return RecurVertiZipfTree(self.environment, self.board_evaluator, self.color, self.arg, board)
