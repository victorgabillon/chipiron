...
def test_representation(
    ...
    parent_node_board_representation: Representation364 = representation_factory.create_from_board(board=board)
    ...
    direct_rep: BoardRepresentation = representation_factory.create_from_board(board=board)
    rep_from_parents: BoardRepresentation = representation_factory.create_from_board_and_from_parent(
        board=board,
...
def test_representation364(
    ...
    board_one: IBoard = create_board(
        ...
        fen_with_history=FenPlusHistory(current_fen='rnb2bnr/ppp2ppp/2k3q1/8/8/1Q3K2/PPP2PPP/RNB2BNR w - - 0 1')
    )

    board_two: IBoard = create_board(
        use_rust_boards=use_rust_boards,
        use_board_modification=use_board_modification,
        fen_with_history=FenPlusHistory(current_fen='rnb2bnr/ppp2ppp/2k3q1/8/8/1Q3K2/PPP2PPP/RNB2BNR b - - 0 1')
    )
    ...
    if use_board_modification:
        board_modification_one: BoardModificationP | None = board_one.play_move_uci(move_uci='a2a3')
        board_modification_two: BoardModificationP | None = board_two.play_move_uci(move_uci='h7h6')
        assert board_modification_one is not None
        assert board_modification_two is not None
        assert (board_one.occupied == bitboard_rotate(board_two.occupied))
        board_representation_one = representation_factory.create_from_board_and_from_parent(
            board=board_one,
            board_modifications=board_modification_one,
            parent_node_board_representation=board_representation_one_copy
        )
        board_representation_two = representation_factory.create_from_board_and_from_parent(
            board=board_two,
            board_modifications=board_modification_two,
            parent_node_board_representation=board_representation_two_copy
        )
    else:
        board_representation_one = representation_factory.create_from_board(board=board_one)
        board_representation_two = representation_factory.create_from_board(board=board_two)
