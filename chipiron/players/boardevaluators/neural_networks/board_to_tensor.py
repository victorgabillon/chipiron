...
def transform_board_pieces_one_side(
    board: BoardChi, requires_grad_bool: bool
) -> torch.Tensor:
    """
    ...
    transform[0] = bin(
        board.chess_board.pawns & board.chess_board.occupied_co[color_turn]
    ).count("1") - bin(
        board.chess_board.pawns & board.chess_board.occupied_co[color_not_turn]
    ).count(
        "1"
    )
    transform[1] = bin(
        board.chess_board.knights & board.chess_board.occupied_co[color_turn]
    ).count("1") - bin(
        board.chess_board.knights & board.chess_board.occupied_co[color_not_turn]
    ).count(
        "1"
    )
    transform[2] = bin(
        board.chess_board.bishops & board.chess_board.occupied_co[color_turn]
    ).count("1") - bin(
        board.chess_board.bishops & board.chess_board.occupied_co[color_not_turn]
    ).count(
        "1"
    )
    transform[3] = bin(
        board.chess_board.rooks & board.chess_board.occupied_co[color_turn]
    ).count("1") - bin(
        board.chess_board.rooks & board.chess_board.occupied_co[color_not_turn]
    ).count(
        "1"
    )
    transform[4] = bin(
        board.chess_board.queens & board.chess_board.occupied_co[color_turn]
    ).count("1") - bin(
        board.chess_board.queens & board.chess_board.occupied_co[color_not_turn]
    ).count(
        "1"
    )
