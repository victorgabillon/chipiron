...
def test_random(use_rust_boards: bool) -> None:
    ...
    for half_move_, board_key_, node_two_ in tree_two.descendants.iter_on_all_nodes():
        node_one_: ITreeNode[Any] = tree_one.descendants.descendants_at_half_move[
            half_move_
        ][board_key_]
        ...
        assert (
            node_one_.id == node_two_.id
        )  # a test for universal behavior that is probably not in the right place!
