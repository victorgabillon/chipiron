...
import chipiron.players.boardevaluators.board_representations as rep


def create_tree_and_value_builders(args: dict,
                                   syzygy,
                                   random_generator: random.Random) -> MoveSelector:
    node_evaluators_wrapper: tree_man.NodeEvaluatorsWrapper = tree_man.create_node_evaluator(
    ...
    board_representation_factory :object = rep.create_board_representation(args= args['board_evaluator'])
    ...
    tree_factory = MoveAndValueTreeFactory(
        ...
        node_evaluator=node_evaluators_wrapper
    )
