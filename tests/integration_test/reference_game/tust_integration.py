...
for config in configs:
    with open(os.path.join('data/reference_games', config, 'inputs_and_parsing/one_match_script_merge.yaml')) as file:
        args_dict = yaml.safe_load(file)

    # Converting the args in the standardized dataclass
    args: MatchArgs = dacite.from_dict(
        data_class=MatchArgs,
        data=args_dict
    )
    ...
    print('match_report.match_move_history, match_report_base.match_move_history',
          match_report.match_move_history, match_report_reference.match_move_history)
