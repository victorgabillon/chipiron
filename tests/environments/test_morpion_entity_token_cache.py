"""Tests for Morpion entity-token tensor cache streaming support."""
...
from chipiron.environments.morpion.players.evaluators.neural_networks.training.entity_token_cache import (
...
def test_default_entity_token_cache_paths_include_token_and_row_limits(
    tmp_path: Path,
) -> None:
    """Default entity-token cache paths should include token and row limits."""
    rows_path = tmp_path / "rows" / "generation_000035.jsonl"

    all_paths = default_entity_token_cache_paths(rows_path, entity_max_tokens=1536)
    limited_paths = default_entity_token_cache_paths(
        rows_path,
    ...
    assert all_paths.tensor_path == (
        ...
        / "rows"
        / "tensor_cache"
        / "generation_000035.entity_tokens.max_tokens_1536.all.pt"
    )
    assert all_paths.manifest_path == (
        tmp_path
        / "rows"
        / "tensor_cache"
        / "generation_000035.entity_tokens.max_tokens_1536.all.manifest.json"
    )
    assert limited_paths.tensor_path.name == (
        "generation_000035.entity_tokens.max_tokens_1536.max_rows_10000.pt"
    )


def test_entity_token_cache_materializes_reuses_and_packs(
    tmp_path: Path,
) -> None:
    """A entity-token cache should build canonical packed CPU tensors."""
    ...
    cached_again = load_or_materialize_entity_token_cache(
        rows_path=rows_path,
        row_chunk_size=2,
        max_rows=3,
        entity_max_tokens=128,
    ...
    assert entity_token_cache_is_valid(
        rows_path=rows_path,
        entity_max_tokens=128,
        max_rows=3,
    )
    assert cached_again.manifest.row_count == 3
    assert cached_again.manifest.entity_max_tokens == 128
...
def test_entity_token_cache_invalidates_when_source_changes(tmp_path: Path) -> None:
    ...
    assert entity_token_cache_is_valid(rows_path=rows_path, entity_max_tokens=128)

    with open(rows_path, "a", encoding="utf-8") as handle:
        handle.write("\n")

    assert not entity_token_cache_is_valid(rows_path=rows_path, entity_max_tokens=128)


def test_entity_token_cache_batch_matches_direct_entity_token_collate(
    tmp_path: Path,
) -> None:
    """Packed cache reconstruction should match direct entity-token collation."""
    ...
    converter = MorpionEntityTokenConverter(dynamics=dynamics, max_tokens=128)
    direct_batch = collate_morpion_entity_token_supervised_samples(
        tuple(
            process_morpion_supervised_row_to_entity_token_tensors(
                rows[row_index],
                dynamics=dynamics,
    ...
    cached_batch = entity_token_cache_batch(cache=cache, row_indices=row_indices)
...
def test_entity_token_cache_handles_empty_rows(tmp_path: Path) -> None:
    """An empty rows artifact should materialize a valid entity-token cache."""
