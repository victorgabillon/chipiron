...
class HumanSelector:
    """Minimal selector exposing itself as human for GUI metadata tests."""
...
class DummyAnemoneArgs:
    """Tiny wrapper matching the field accessed by `format_player_label`."""
...
def test_make_players_info_payload_keeps_white_black_mapping_and_current_labels() -> None:
    ...
    white_player = PlayerFactoryArgs(
        player_args=PlayerArgs(name="GuiHuman", main_move_selector=HumanSelector()),
        seed=1,
    )
    black_player = PlayerFactoryArgs(
        player_args=PlayerArgs(
            name="TreeBot",
            main_move_selector=TreeAndValueAppArgs(
                anemone_args=DummyAnemoneArgs(
                    stopping_criterion=TreeBranchLimitArgs(tree_branch_limit=64)
                )
            ),
        ),
        seed=1,
