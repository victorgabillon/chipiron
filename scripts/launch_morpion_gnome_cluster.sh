...
TRAINING_ARGS="--pipeline-stage training_worker $ROLLOUT_ARGS"
if [[ -n "$MORPION_TRAINING_EVALUATOR_NAMES" ]]; then
  TRAINING_ARGS="$TRAINING_ARGS --training-evaluator-names $(printf '%q' "$MORPION_TRAINING_EVALUATOR_NAMES")"
fi
if [[ -n "$MORPION_TRAINING_MAX_ROWS" ]]; then
  TRAINING_ARGS="$TRAINING_ARGS --training-max-rows $(printf '%q' "$MORPION_TRAINING_MAX_ROWS")"
fi
