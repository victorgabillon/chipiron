...
MORPION_MIN_AVAILABLE_RAM_MB="${MORPION_MIN_AVAILABLE_RAM_MB:-}"
...
GROWTH_ARGS="--pipeline-stage growth --tree-branch-limit $GROWTH_TREE_BRANCH_LIMIT --max-growth-steps-per-cycle $GROWTH_MAX_STEPS_PER_CYCLE --save-after-tree-growth-factor $GROWTH_SAVE_AFTER_TREE_GROWTH_FACTOR --save-after-seconds $GROWTH_SAVE_AFTER_SECONDS --candidate-checkpoint-load-headroom-factor $MORPION_CANDIDATE_CHECKPOINT_LOAD_HEADROOM_FACTOR --candidate-checkpoint-load-min-headroom-mb $MORPION_CANDIDATE_CHECKPOINT_LOAD_MIN_HEADROOM_MB $ROLLOUT_ARGS $RAM_GUARD_ARGS $GROWTH_PROFILE_ARGS"
