...
launch_worker_terminal() {
  ...
  local command="cd \"$REPO_ROOT\" && export PYTHONPATH=src && export MORPION_WORK_DIR=\"$MORPION_WORK_DIR\" && trap 'echo; echo \"[$worker_name] stopped; terminal kept open\"; exec bash' INT TERM && printf '%b\\n' \"$startup_message\" && while true; do ${extra_prefix}python -m chipiron.environments.morpion.bootstrap.launcher --work-dir \"$MORPION_WORK_DIR\" --pipeline-mode artifact_pipeline ${launcher_args} 2>&1 | tee -a \"$LOG_DIR/$log_name\"; status=\${PIPESTATUS[0]}; echo \"[$worker_name] worker exited with status \$status; restarting in $sleep_seconds s\"; sleep $sleep_seconds; done"
