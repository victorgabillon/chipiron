...
GROWTH_ARGS="--pipeline-stage growth --tree-branch-limit $GROWTH_TREE_BRANCH_LIMIT --max-growth-steps-per-cycle $GROWTH_MAX_STEPS_PER_CYCLE --save-after-tree-growth-factor $GROWTH_SAVE_AFTER_TREE_GROWTH_FACTOR --save-after-seconds $GROWTH_SAVE_AFTER_SECONDS $ROLLOUT_ARGS $RAM_GUARD_ARGS"
DATASET_ARGS="--pipeline-stage dataset_worker $ROLLOUT_ARGS $RAM_GUARD_ARGS"
TRAINING_ARGS="--pipeline-stage training_worker --training-row-chunk-size $MORPION_TRAINING_ROW_CHUNK_SIZE $ROLLOUT_ARGS $RAM_GUARD_ARGS"
