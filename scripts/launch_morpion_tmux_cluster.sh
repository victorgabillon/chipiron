...
status_loop="$shared_env && echo \"[STATUS] work_dir=$MORPION_WORK_DIR\" && echo \"[STATUS] refresh_seconds=$MORPION_CLUSTER_STATUS_REFRESH_SECONDS\" && \"$PYTHON_BIN\" -m chipiron.environments.morpion.bootstrap.training_recap --work-dir \"$MORPION_WORK_DIR\" --watch \"$MORPION_CLUSTER_STATUS_REFRESH_SECONDS\" --clear"
