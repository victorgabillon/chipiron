...
growth_loop="$shared_env && echo \"[GROWTH] work_dir=$MORPION_WORK_DIR\" && echo \"[GROWTH] anemone_repo_root=$ANEMONE_REPO_ROOT\" && echo \"[GROWTH] training_export_mode=$TRAINING_EXPORT_MODE\" && $python_diagnostics && while true; do \"$PYTHON_BIN\" -m chipiron.environments.morpion.bootstrap.launcher --work-dir \"$MORPION_WORK_DIR\" --pipeline-mode artifact_pipeline --training-export-mode \"$TRAINING_EXPORT_MODE\" --pipeline-stage growth 2>&1 | tee -a \"$LOG_DIR/growth.log\"; echo \"[GROWTH] worker exited with status \\\$?; restarting in $GROWTH_SLEEP_SECONDS s\"; sleep $GROWTH_SLEEP_SECONDS; done"
dataset_loop="$shared_env && echo \"[DATASET] work_dir=$MORPION_WORK_DIR\" && echo \"[DATASET] anemone_repo_root=$ANEMONE_REPO_ROOT\" && echo \"[DATASET] training_export_mode=$TRAINING_EXPORT_MODE\" && $python_diagnostics && while true; do \"$PYTHON_BIN\" -m chipiron.environments.morpion.bootstrap.launcher --work-dir \"$MORPION_WORK_DIR\" --pipeline-mode artifact_pipeline --training-export-mode \"$TRAINING_EXPORT_MODE\" --pipeline-stage dataset_worker 2>&1 | tee -a \"$LOG_DIR/dataset.log\"; echo \"[DATASET] worker exited with status \\\$?; restarting in $DATASET_SLEEP_SECONDS s\"; sleep $DATASET_SLEEP_SECONDS; done"
training_loop="$shared_env && echo \"[TRAINING] work_dir=$MORPION_WORK_DIR gpu=0\" && echo \"[TRAINING] anemone_repo_root=$ANEMONE_REPO_ROOT\" && echo \"[TRAINING] training_export_mode=$TRAINING_EXPORT_MODE\" && $python_diagnostics && while true; do CUDA_VISIBLE_DEVICES=0 \"$PYTHON_BIN\" -m chipiron.environments.morpion.bootstrap.launcher --work-dir \"$MORPION_WORK_DIR\" --pipeline-mode artifact_pipeline --training-export-mode \"$TRAINING_EXPORT_MODE\" --pipeline-stage training_worker 2>&1 | tee -a \"$LOG_DIR/training.log\"; echo \"[TRAINING] worker exited with status \\\$?; restarting in $TRAINING_SLEEP_SECONDS s\"; sleep $TRAINING_SLEEP_SECONDS; done"
reevaluation_loop="$shared_env && echo \"[REEVALUATION] work_dir=$MORPION_WORK_DIR gpu=1\" && echo \"[REEVALUATION] anemone_repo_root=$ANEMONE_REPO_ROOT\" && echo \"[REEVALUATION] training_export_mode=$TRAINING_EXPORT_MODE\" && $python_diagnostics && while true; do CUDA_VISIBLE_DEVICES=1 \"$PYTHON_BIN\" -m chipiron.environments.morpion.bootstrap.launcher --work-dir \"$MORPION_WORK_DIR\" --pipeline-mode artifact_pipeline --training-export-mode \"$TRAINING_EXPORT_MODE\" --pipeline-stage reevaluation 2>&1 | tee -a \"$LOG_DIR/reevaluation.log\"; echo \"[REEVALUATION] worker exited with status \\\$?; restarting in $REEVALUATION_SLEEP_SECONDS s\"; sleep $REEVALUATION_SLEEP_SECONDS; done"
