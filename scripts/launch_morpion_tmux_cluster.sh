...
reevaluation_loop="$shared_env && echo \"[REEVALUATION] work_dir=$MORPION_WORK_DIR gpu=1\" && echo \"[REEVALUATION] anemone_repo_root=$ANEMONE_REPO_ROOT\" && echo \"[REEVALUATION] training_export_mode=$TRAINING_EXPORT_MODE\" && $rollout_echo && $python_diagnostics && reevaluation_idle_checks=0 && while true; do CUDA_VISIBLE_DEVICES=1 \"$PYTHON_BIN\" -m chipiron.environments.morpion.bootstrap.launcher --work-dir \"$MORPION_WORK_DIR\" --pipeline-mode artifact_pipeline --training-export-mode \"$TRAINING_EXPORT_MODE\" --pipeline-stage reevaluation --quiet-worker-startup $rollout_args 2>&1 | tee -a \"$LOG_DIR/reevaluation.log\"; status=\${PIPESTATUS[0]}; if [[ \"\$status\" -eq 0 ]]; then reevaluation_idle_checks=\$((reevaluation_idle_checks + 1)); if (( reevaluation_idle_checks == 1 || reevaluation_idle_checks % 12 == 0 )); then echo \"[REEVALUATION] idle; next check in $REEVALUATION_SLEEP_SECONDS s\"; fi; sleep $REEVALUATION_SLEEP_SECONDS; continue; fi; echo \"[REEVALUATION] ERROR worker exited status=\$status; restarting in $REEVALUATION_SLEEP_SECONDS s\"; sleep $REEVALUATION_SLEEP_SECONDS; done"
